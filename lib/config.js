...
function loadConfig({ specFile } = {}) {
  ...
  const resource_categories= yaml.safeLoad(resourcesConfigFile);
...
function createDefinitions(config = {}) {
  ...
  for(const name in openApiSpec.definitions) {
    ...
    //console.log(items);
    config.definitions.add(item);
  }

  config.definitions.initialize();

  const k = config.definitions.all();
  Object.values(k).forEach(function(v) {
    //console.log(JSON.stringify(v._attributes(), null, 2));
  });
