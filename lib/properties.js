...
const resolveRef = ({ data = {}, definitions = {}, resolve = '', packageMap = {} } = {}) => {
  if(!data['$ref'])
    return data;

  const ref = data['$ref'].replace('#/definitions/', '');
  const gvk = getKindGroupVersion(ref, packageMap);

  // Avoid resolving every $ref for core
  const groupIsCore = ({ group, kind }) => (group == 'core' && ! /(Spec|Status)$/.test(kind));

  switch (typeof (resolve)) {

    case 'string':
      // Only resolve for specified group
      if(groupIsCore({ group: gvk.group, kind: gvk.kind }) || gvk.group != resolve) {
        Object.assign(data, { gvk, type: `${gvk.kind} ${gvk.group}/${gvk.version}` });
        return data;
      }
      break;
...
function flattenProps({ data = {}, definitions = {}, propName = '.', resolve = true, packageMap = {} } = {}) {
  let obj = {};
  const keys = [
    '$ref',
    'description',
  ...
  const flatten = (accum, v) => {
    ...
    accum = {
      ...accum,
      ...flattenProps({ data: value, propName: prop, definitions, resolve, packageMap })
    };
    return accum;
  };
  ...
  if(data['$ref'] && !data['$ref'].includes('JSONSchemaProps')) {
    data = resolveRef({ data, definitions, resolve, packageMap });
  }
