const getGroupVersionKind = require('./util').guessGroupVersionKind;
...
const resolveRef = ({ data, definitions, resolve } = {}) => {
  ...
  const gvk = getGroupVersionKind(ref);
...
function flattenSpec({ data, definitions = {}, propName = '.', resolve = true } = {}) {
  let obj = {};
  const keys = [
    '$ref',
    'description',
  ...
  const flatten = (accum, v) => {
    ...
    accum = {
      ...accum,
      ...flattenSpec({ data: value, propName: prop, definitions, resolve })
    };
    return accum;
  };
...
flattenSpec.resolveRef = resolveRef;
