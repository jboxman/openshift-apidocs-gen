...
function resourceVisitor({ resource, definition }) {
  if(definition) {
    ...
    console.error(`No known definition for '${resource.name} [${resource.group}/${resource.version}]'`);
    //process.exit(1);
  }
}
...
function createDefinitions(config = {}) {
  ...
  for(const [ ref, definition ] of Object.entries(openApiSpec.definitions)) {
    ...
    // TODO - While the above are resources with REST endpoints,
    // still need to load all other definitions;
    // Some of these are relatedResources, and they are necessary too.
    else {
      ...
      flatSpec = flattenSpec({ data: definition, definitions: openApiSpec.definitions, resolve: true });
      //wantPaths = ['.'];
      wantPaths = Object.entries(flatSpec).reduce((accum, entry) => {
        if(['object', 'array'].includes(entry[1].type))
          accum.push(entry[0]);
