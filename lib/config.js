...
function resourceVisitor({ resource, definition }) {
  if(definition) {
    ...
    // TODO - Use a function
    //const wantPaths = [ '.', '.metadata', '.status', '.spec' ];
    const wantPaths = Object.entries(resource.flatSpec).reduce((accum, entry) => {
    ...
    const propertiesByPath = wantPaths.reduce((accum, path) => {
      return {
        ...accum,
        [path]: getPropertiesByPath({ properties: resource.flatSpec, otherPaths: wantPaths, reqPath: path })
      };
    }, {});

    resource.specsByPath = propertiesByPath;
  }
  else {
